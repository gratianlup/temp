...
???+ note
    If the trace fails to load, the *Trace view report* windows is automatically opened. The *Modules tab* lists the modules referenced by the trace and indicates whether the binary and debug info symbols could be found and loaded. See the [Trace view report](trace-overview.md) page for more details.
